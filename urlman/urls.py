...
class Urls(object):
    ...
    def get_url(self, attr, strip_host=True):
        # Get the URL value
        try:
            url = self.urls[attr]
        except KeyError:
...
class UrlString(str):
    """
    Special string subclass for URLs (with have with/without host modes)
    """

    def __new__(self, with_host, without_host):
        return str.__new__(self, without_host)

    def __init__(self, with_host, without_host):
        self.with_host = with_host
        self.without_host = without_host
    ...
    def get_value(self, key, args, kwargs):
        ...
        try:
            if self.example:
                ...
                value = self.urls.get_url(key, strip_host=False)
        except ValueError:
            pass
        else:
